...
%     plot(range_arr{t}, zline_arr{t})

end

%% Get user input as to which azimuths to exclude and remove them
...
tick     = (10:10:360);
tickline = base_rad*(1e-3)*[cosd(tick); sind(tick)];

xstart = deg2km(volc_lon)*(1e-3)*ones(size(tick));
ystart = deg2km(volc_lat)*(1e-3)*ones(size(tick));

plot([xstart; xstart + tickline(1,:)], [ystart; ystart + tickline(2,:)], 'k');
plot([xstart; xstart + base_rad*(1e-3)], [ystart; ystart], 'm')
text(xstart + tickline(1,:), ystart + tickline(2,:), string(tick));

% User Input

excludes = input('Input an array of azimuth angles (1 -> 360) to exlude from processing. \n');
theta    = setdiff(theta, excludes);

% Remove exlcuded profiles from data

range_arr = range_arr(theta);
zline_arr = zline_arr(theta);
pXYZ = pXYZ(theta, :);
...
tmask = ~ismember(theta, unique(high_flag));
