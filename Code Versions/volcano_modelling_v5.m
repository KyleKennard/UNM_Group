...
dat_file = dir(strcat(volcano,'/',volcano, '.dat'));
...
line_adj = 1.2;   % frac. change from Rbase for extending profile lines
...
disp = sprintf(' RMS: %.1d line adj: %.1d', [RMS,line_adj]);
figtitle=strcat('Volcano ',volcano,disp);
...
%% Save Fit Figure and Goodness of Fit
figsave_var = sprintf('RMS%.1d_lineadj%.1d',[RMS,line_adj]);
figsave = strcat(volcano,figsave_var);
sv_fig=strcat(dted_file.folder, '\',figsave,'.fig');
savefig(4,sv_fig);

sv_dat = strcat(dted_file.folder, '\', volcano, '_Good_of_fit', '.dat');

%RUN THIS IF THERE IS NO FILE CREATED TO STORE GOODNESS OF FIT YET
% good_fit=[line_adj;RMS];
% save(sv_dat, 'good_fit');

%RUN THIS IF THE FILE FOR GOODNESS OF FIT HAS ALREADY BEEN CREATED
good_fit = importdata(sv_dat);
good_fit(:,end+1)=[line_adj,RMS];
save(sv_dat, 'good_fit');
